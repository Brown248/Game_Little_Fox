 ...
 *
 *  Returns null when it belongs to someone else, when it was saved against a
 *  different number of blocks, or when it is finished or malformed — a bad
 *  resume is worse than no resume. */
export function loadProgress(playerId: string, blockCount: number): SavedRun | null {
  try {
