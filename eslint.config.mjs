...
const config = [
...

  {
    ...
    // effect body. These five screens have to do exactly that and cannot be
    // rewritten around it:
    //   · StartForm · PlayClient · both leaderboards — read the saved player
    //     out of localStorage after mount. Doing it during render would break
    //     SSR: the server has no localStorage, so the markup would not match.
    //   · the leaderboards and PlayClient also kick off their Supabase query
    //     on mount, and that query reports its own loading/failure state.
