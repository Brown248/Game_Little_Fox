...
เกมเป็นรอบเดียว 50 ข้อ แต่ห้องเรียนไปถึงทีละ Part — **เด็กที่เรียนถึงแค่ Part แรกจะเจออีก 35 ข้อ
...
- ⚠️ **บอร์ดเรียงด้วยคะแนนดิบ** คนที่หยุดที่ 15 ข้อ (เต็ม 150) จึงไม่มีทางขึ้นเหนือคนที่ทำครบ 50 ข้อ
  (เต็ม 500) — **ยอมรับข้อนี้แล้ว** เพราะเล่นซ้ำได้ พอเรียนเพิ่มแล้วกลับมาเล่นใหม่คะแนนก็ขยับเอง
- ปุ่มบนหัวชื่อ `Finish` · ปุ่มยืนยันในกล่องชื่อ **`Finish now`** — ตั้งใจให้ชื่อไม่ซ้ำกัน

### เล่นค้างไว้ได้ (`lib/progress.ts`)
เกมเดียว 50 ข้อ ~12–20 นาที และคะแนนบันทึกตอนจบ **ครั้งเดียว** — ปิดแท็บตอนข้อ 40 คือเสียทั้งหมด
...
- **ใบเซอร์ตัดสินจาก "รอบที่เล่นจบที่ดีที่สุด" ไม่ใช่ "รอบที่คะแนนสูงสุด"** (`certificateRun` ใน `RankBoard.tsx`)
  เล่นครบ 50 ข้อถูก 30 = ได้ใบ (ขั้นต่ำคือ 25) · กลับมาเล่นใหม่ตอบ 40 ข้อแล้วกด Finish = คะแนนสูงกว่าแต่เล่นไม่จบ
...
- **ใครได้ certificate:** ต้อง **เล่นครบทั้งเกม** (กด Finish กลางคันไม่ได้) **และตอบถูก ≥ ครึ่งหนึ่งของจำนวนข้อ**
  ...
  ไม่งั้นเด็กที่ทำ 12 จาก 15 ข้อแล้วกด Finish จะได้ 80% แล้วคว้าใบเซอร์ไปทั้งที่เล่นแค่ Part เดียวจาก 50 ข้อ
...
> ⚠️ **ยูนิตไม่ใช่สิ่งที่เด็กเลือกอีกแล้ว** — ทั้งสองไฟล์ต่อกันเป็น **เกมเดียว 50 ข้อ / 500 คะแนน**
...
- **⚠️ ไม่มี "ภาพวาดสัตว์" เหลือในโปรเจกต์แล้ว — ห้ามเอากลับ** ครูสั่ง
  ...
  - เกมรวม **50 ข้อ / 500 คะแนน** (unit-01 15 + unit-02 35)
