...
### `GAME_ID = "game-03"` — อ่านให้จบก่อนแก้
...
- **แก้เนื้อหาจนจำนวนข้อเปลี่ยน = bump เลขต่อไป** เพื่อเปิดบอร์ดใหม่ (ทำมาแล้ว 2 รอบ: 77 → 50 → 47 ข้อ)
...
เกมเป็นรอบเดียว 47 ข้อ แต่ห้องเรียนไปถึงทีละ Part — **เด็กที่เรียนถึงแค่ Part แรกจะเจออีก 32 ข้อ
...
- ⚠️ **บอร์ดเรียงด้วยคะแนนดิบ** คนที่หยุดที่ 15 ข้อ (เต็ม 150) จึงไม่มีทางขึ้นเหนือคนที่ทำครบ 47 ข้อ
  (เต็ม 470) — **ยอมรับข้อนี้แล้ว** เพราะเล่นซ้ำได้ พอเรียนเพิ่มแล้วกลับมาเล่นใหม่คะแนนก็ขยับเอง
- ปุ่มบนหัวชื่อ `Finish` · ปุ่มยืนยันในกล่องชื่อ **`Finish now`** — ตั้งใจให้ชื่อไม่ซ้ำกัน

### เล่นค้างไว้ได้ (`lib/progress.ts`)
เกมเดียว 47 ข้อ ~12–20 นาที และคะแนนบันทึกตอนจบ **ครั้งเดียว** — ปิดแท็บตอนข้อ 40 คือเสียทั้งหมด
...
- **Listening:** ถ้าไม่มี `audioUrl` (หรือโหลดไฟล์ไม่ได้) จะ fallback ไป browser TTS · path relative ใน JSON จะถูกต่อ prefix `/audio/` · ใส่ URL เต็มของ Supabase Storage ก็ได้
  - ⚠️ **ทุกข้อที่ ship ต้องมี mp3 จริง** — เคยมี 3 ข้อที่ไม่มีไฟล์แล้วปล่อยให้เครื่องอ่าน
    ครูได้ยินแล้วถามว่า "ทำไมครูบอกมีเสียงบอทอยู่" → **ตัดออกดีกว่าปล่อยให้เป็นเสียงหุ่นยนต์**
    เทส `"lists every clue that falls back to the device voice"` บังคับให้ `withoutAudio === 0`
- **Writing:** เก็บแค่ "ทำเสร็จ" ไม่เก็บข้อความที่พิมพ์ (ตั้งใจ) และไม่คิดคะแนน
  - **เหลือ 5 ข้อ** — ครูเปลี่ยนจากช่องเติมประโยค 17 ช่อง (spirit animal 7 + Speaking Time 10)
    เป็นคำถามปลายเปิด 5 ข้อ ("Part เขียนปรับให้เหลือแค่นี้พอ") · **ไม่กระทบจำนวนข้อที่คิดคะแนน**
    เพราะ writing ไม่คิดคะแนนอยู่แล้ว จึงไม่ต้อง bump `GAME_ID`
- **ใบเซอร์ตัดสินจาก "รอบที่เล่นจบที่ดีที่สุด" ไม่ใช่ "รอบที่คะแนนสูงสุด"** (`certificateRun` ใน `RankBoard.tsx`)
  เล่นครบ 47 ข้อถูก 30 = ได้ใบ (ขั้นต่ำคือ 24) · กลับมาเล่นใหม่ตอบ 40 ข้อแล้วกด Finish = คะแนนสูงกว่าแต่เล่นไม่จบ
...
- **ใครได้ certificate:** ต้อง **เล่นครบทั้งเกม** (กด Finish กลางคันไม่ได้) **และตอบถูก ≥ ครึ่งหนึ่งของจำนวนข้อ**
  ...
  ไม่งั้นเด็กที่ทำ 12 จาก 15 ข้อแล้วกด Finish จะได้ 80% แล้วคว้าใบเซอร์ไปทั้งที่เล่นแค่ Part เดียวจาก 47 ข้อ
...
|-------|------|-----------------|
| `unit-01` **Animal Words** | unscramble **อิโมจิสีปกติ** 15 ข้อ | **15 / 150** |
| `unit-02` **Wild Life and Wonderful Creatures** | B quiz 10 · C1 เสียงสัตว์ 5 · C2 เรียงประโยค 10 · D listening **7 (mp3 จริงครบทุกข้อ)** · E writing **5** (ไม่คิดคะแนน) | **32 / 320** |

> ⚠️ **ยูนิตไม่ใช่สิ่งที่เด็กเลือกอีกแล้ว** — ทั้งสองไฟล์ต่อกันเป็น **เกมเดียว 47 ข้อ / 470 คะแนน**
...
- **⚠️ Part 1 ไม่มี "เงา" แล้ว — อิโมจิโชว์สีปกติตั้งแต่เปิดข้อ** ครูสั่ง "ไม่เอา Shadow เอาแค่ภาพสีอิโมจิพอ"
  ...
  - ⚠️ **ภาพที่ครูส่งมาให้เลือกข้อเป็นภาพเวอร์ชันเก่าที่ยังมีเงา** ใช้ดูว่า "เอาข้อไหนไว้" เท่านั้น
    ครูกำชับเองว่า "ห้ามใส่ Shadow เข้ามาเด็ดขาด"
- **Part 1 เหลือ 15 ข้อ** — ครูเล่นจริงแล้วแคปมา 15 ใบ สั่งว่า "เอาแค่นี้พอ...ที่เหลือเอาออกเพราะเยอะเกิน"
  ที่เหลือคือ elephant · lion · tiger · zebra · monkey · panda · fox · wolf · rabbit · kangaroo ·
  crocodile · snake · butterfly · dolphin · octopus
  ตัดออก 12 ตัว: gorilla · deer · bear · camel · koala · turtle · frog · eagle · owl · penguin · whale · shark
- **⚠️ ไม่มี "ภาพวาดสัตว์" เหลือในโปรเจกต์แล้ว — ห้ามเอากลับ** ครูสั่ง
  ...
  - เกมรวม **47 ข้อ / 470 คะแนน** (unit-01 15 + unit-02 32)
...
**เหลืองานเนื้อหา:** ให้ครูฟังยืนยันคำใบ้ข้อ 7 (Yeti) · ยูนิตอื่นๆ เพิ่มเติม

**3 ข้อของ Part D ที่ตัดออกเพราะไม่มี mp3 — เอากลับได้ทันทีที่มีไฟล์เสียง**
(วาง mp3 ที่ `public/audio/unit-02/clue-8.mp3` ฯลฯ แล้วเติมกลับท้ายบล็อก `listening`
· จำนวนข้อจะเปลี่ยน = ต้อง bump `GAME_ID` เป็น `game-04`)

| คำตอบ | คำใบ้ | ตัวเลือก |
|-------|------|---------|
| Phoenix | I am a giant bird. When I die, I become ashes. Then I am born again. | Griffin · **Phoenix** · Dragon |
| Fairy | I am a mini human with wings. I live deep in the forest. I protect the living things. | Mermaid · **Fairy** · Naga |
| Mermaid | I am half woman and half fish. I live deep in the ocean. I sing beautiful songs. | **Mermaid** · Elf · Naga |
